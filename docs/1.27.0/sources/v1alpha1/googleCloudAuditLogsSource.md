---
permalink: /1.27.0/sources/v1alpha1/googleCloudAuditLogsSource/
---

# sources.v1alpha1.googleCloudAuditLogsSource

"TriggerMesh event source for Google Cloud Audit Logs."
...
* [`obj spec`](#obj-spec)
  * [`fn withMethodName(methodName)`](#fn-specwithmethodname)
  * [`fn withResourceName(resourceName)`](#fn-specwithresourcename)
  ...
  * [`obj spec.auth`](#obj-specauth)
    * [`fn withGcpServiceAccount(gcpServiceAccount)`](#fn-specauthwithgcpserviceaccount)
    * [`fn withKubernetesServiceAccount(kubernetesServiceAccount)`](#fn-specauthwithkubernetesserviceaccount)
    * [`obj spec.auth.serviceAccountKey`](#obj-specauthserviceaccountkey)
      * [`fn withValue(value)`](#fn-specauthserviceaccountkeywithvalue)
      * [`obj spec.auth.serviceAccountKey.valueFromSecret`](#obj-specauthserviceaccountkeyvaluefromsecret)
        * [`fn withKey(key)`](#fn-specauthserviceaccountkeyvaluefromsecretwithkey)
        * [`fn withName(name)`](#fn-specauthserviceaccountkeyvaluefromsecretwithname)
  * [`obj spec.pubsub`](#obj-specpubsub)
    * [`fn withProject(project)`](#fn-specpubsubwithproject)
    * [`fn withTopic(topic)`](#fn-specpubsubwithtopic)
...
new returns an instance of GoogleCloudAuditLogsSource
...
### fn spec.withMethodName

```ts
withMethodName(methodName)
```

"The name of the service method or operation. For API calls, this should be the name of the API method. For example, \"google.pubsub.v1.Publisher.CreateTopic\"."

### fn spec.withResourceName

```ts
withResourceName(resourceName)
```

"The resource or collection that is the target of the operation. The name is a scheme-less URI, not including the API service name. https://cloud.google.com/logging/docs/reference/audit/auditlog/rest/Shared.Types/AuditLog"
...
"The name of the API service performing the operation. For example, \"pubsub.googleapis.com\"."
...
"Authentication options for Google Cloud Platform API."

### fn spec.auth.withGcpServiceAccount

```ts
withGcpServiceAccount(gcpServiceAccount)
```

"GCP Service account name to impersonate Identity and Access Management (IAM) service accounts to access Google Cloud services. For the GKE platform only. More details about workload identity at https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity"

### fn spec.auth.withKubernetesServiceAccount

```ts
withKubernetesServiceAccount(kubernetesServiceAccount)
```
...
## obj spec.auth.serviceAccountKey

"Service account key used to authenticate the event source and allow it to interact with Google Cloud APIs. Only the JSON format is supported."

### fn spec.auth.serviceAccountKey.withValue

```ts
withValue(value)
```

"Literal value of the service account key."

## obj spec.auth.serviceAccountKey.valueFromSecret

"A reference to a Kubernetes Secret object containing the service account key."

### fn spec.auth.serviceAccountKey.valueFromSecret.withKey
...
### fn spec.auth.serviceAccountKey.valueFromSecret.withName
...
"Attributes related to the configuration of Pub/Sub resources associated with the Cloud Audit Log."

### fn spec.pubsub.withProject

```ts
withProject(project)
```

"Name of the GCP project where Pub/Sub resources associated with the Cloud Audit log sink are to be created. Mutually exclusive with 'topic' which, if supplied, already contains the project name."

### fn spec.pubsub.withTopic

```ts
withTopic(topic)
```

"Full resource name of the Pub/Sub topic where change notifications originating from the configured audit logs sink are sent to, before being retrieved by this event source. If not supplied, a topic is created on behalf of the user, in the GCP project referenced by the 'project' attribute. The expected format is described at https://cloud.google.com/pubsub/docs/admin#resource_names"
